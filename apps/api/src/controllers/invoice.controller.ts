...
export class InvoiceController {
  ...
  async updateInvoice(req: Request, res: Response, next: NextFunction) {
    ...
    try {
      ...
      if (addRecurringDate) {
        ...
        const offsetMinutes = 7 * 60;
        newNextInvoiceDate.setMinutes(
          newNextInvoiceDate.getMinutes() + offsetMinutes,
        );
  ...
  async downloadInvoice(req: Request, res: Response, next: NextFunction) {
    ...
    try {
      ...
      for (const invoice of selectedInvoices) {
        ...
        await page.setContent(content(invoice), { waitUntil: 'networkidle0' }); // Wait until the network is idle (optional)
  ...
  async sendInvoice(req: Request, res: Response, next: NextFunction) {
    ...
    try {
      ...
      await page.setContent(content(findInvoice));
